...
export function InputBar() {
  ...
  const autoSendOnReleaseRef = useRef(false)
  const ctrlPTTActiveRef = useRef(false)
  ...
  const pressedOtherKeysRef = useRef(false)
  ...
  const cancelRecording = useCallback(() => {
    autoSendOnReleaseRef.current = false
    ctrlPTTActiveRef.current = false
  ...
  const startRecording = useCallback(async () => {
    setVoiceError(null)
    chunksRef.current = []
    const runId = ++recordingRunIdRef.current
    ...
    recorder.onstop = async () => {
      ...
      if (runId !== recordingRunIdRef.current) { setVoiceState('idle'); return }
      if (cancelledRef.current) { cancelledRef.current = false; setVoiceState('idle'); setInput(startingInput); return }
    ...
    pollInterval = setInterval(async () => {
      if (chunksRef.current.length === 0 || cancelledRef.current || mediaRecorderRef.current?.state !== 'recording') return;
      try {
        const blob = new Blob(chunksRef.current, { type: mimeType })
        const wavBase64 = await blobToWavBase64(blob).catch(() => null)
        if (wavBase64) {
          const result = await window.clui.transcribeAudio(wavBase64)
          if (runId !== recordingRunIdRef.current) return
          if (!result.error) {
            const transcript = (result.transcript || (result as any).text || '').trim()
            if (transcript && !cancelledRef.current && mediaRecorderRef.current?.state === 'recording') {
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      if (e.key !== 'Control') {
        if (ctrlPTTActiveRef.current) {
          pressedOtherKeysRef.current = true
          cancelRecording()
        }
    ...
    const onKeyUp = (e: KeyboardEvent) => {
      if (e.key === 'Control') {
        ...
        if (!ctrlPTTActiveRef.current) return
        ctrlPTTActiveRef.current = false
        if (mediaRecorderRef.current?.state === 'recording') {
          if (pressedOtherKeysRef.current) {
            cancelRecording()
  ...
  return (
    <div ref={wrapperRef} data-clui-ui className="flex flex-col w-full relative">
      ...
      <div className="w-full" style={{ minHeight: 62 }}>
        {isMultiLine ? (
          ...
          <div className="flex items-center w-full" style={{ minHeight: 62 }}>
            <textarea
              ref={textareaRef}
              value={input}
              onChange={handleInputChange}
