...
export type NormalizedEvent =
  ...
  | { type: 'tool_call_complete'; index: number; output?: string }
  | { type: 'reasoning_step'; text: string }
  | { type: 'task_switch'; previousTask: string; newTask: string; reason: string; necessary: boolean }
...
export type RiskLevel = 'safe' | 'caution' | 'dangerous' | 'nuclear'
...
export type OperationSnapshot =
  ...
  | { FileModified: { path: string; original_content: string } }
  | { FileDeleted: { path: string; original_content: string } }
