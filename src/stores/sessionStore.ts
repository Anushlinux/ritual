    ...
    set((s) => {
      ...
      const tabs = s.tabs.map((tab) => {
        ...
        switch (event.type) {
          ...
          case 'reasoning_step':
          case 'task_switch':
            // These are attached to the next tool_call by the shim.
            break
          ...
          case 'tool_call_complete': {
            ...
            if (runningTool) {
              ...
              if (event.output) {
                runningTool.content = event.output
              }
            }
          ...
          case 'permission_request': {
            const newReq: import('../types').PermissionRequest = {
      ...
      tabs: s.tabs.map((t) =>
        t.id === tabId
          ? {
              ...
              ...(newStatus === 'idle' ? { currentActivity: '', permissionQueue: [] as import('../types').PermissionRequest[], permissionDenied: null } : {}),
            }
          : t
      ),
    }))
